'''
先在../../PaddleDetection中创建newdata文件夹
原coco数据集放在../../PaddleDetection/aluminum
'''
...
CKimg_dir = '../../PaddleDetection/newdata/images'
CKanno_dir = '../../PaddleDetection/newdata/annotations'
...
def showbycv(coco, dataType, img, classes, origin_image_dir, verbose=False):
    filename = img['file_name'][44:]  # xzyxzy根据coco数据集中的文件名修改
    print( 'filename',filename)
    # filepath = os.path.join(origin_image_dir, dataType, filename)  # xzy
    filepath = origin_image_dir + '/'+filename  #xzy???
...
def get_CK5(origin_anno_dir, origin_image_dir, verbose=False):
    dataTypes = ['train', 'valid']  #xzy
    for dataType in dataTypes:
        annFile = '{}.json'.format(dataType)  #xzy
...
def main():
    base_dir = '../../PaddleDetection/newdata'  # step1 这里是一个新的文件夹，存放转换后的图片和标注xzy
    image_dir = os.path.join(base_dir, 'images')  # 在上述文件夹中生成images，annotations两个子文件夹xzy
    anno_dir = os.path.join(base_dir, 'annotations')  # xzy
    mkr(image_dir)
    mkr(anno_dir)
    origin_image_dir = '../../PaddleDetection/aluminum/images'  # step 2原始的coco的图像存放位置xzy
    origin_anno_dir = '../../PaddleDetection/aluminum/annotations'  # step 3 原始的coco的标注存放位置xzy
